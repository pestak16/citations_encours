...
class Form
{
     ...
     * @param array  $options    An associative array of options for the select element. Each option should have a 'name' and 'value' key.
     */
    public function addSelect(array $attributes, string $label = '', array $options)
    {
        $el = new FormSelect($attributes);

        $el->label($label);
        $el->addOptions($options);
